...
export default class SubmitPayTrail extends React.Component {
  ...
  executePayment(initializedTransaction){
    if(!this.onceOnly){
      this.onceOnly = true;
      this.props.actions.buyDelayed(initializedTransaction)
    }
  }
    ...
    return(
      <div className="container checkout-container">
        <div className="content-container">
          ...
          {this.renderCartInfo()}
          <button className="btn-small btn-blue mobile-full margin-bottom" onClick={() => this.executePayment(this.props.shopItems.initializedTransaction)}>Vahvista osto</button>
        </div>
      </div>
    )
  }
